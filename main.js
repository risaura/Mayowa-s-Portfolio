...
const App = {
    ...
    drawCharacter(t, cam) {
        ...
        // Hair
        ctx.fillStyle = '#1a1a1a';
        px(-5, -22, 10, 6); px(-6, -20, 12, 4); px(-6, -16, 2, 9); px(4, -16, 2, 9); px(-6, -16, 2, 12);

        // Red hair accessory
        ctx.fillStyle = '#e53935'; px(-5, -19, 2, 2);

        // Face
        ctx.fillStyle = '#8D6E4C'; px(-4, -16, 8, 8);
        ctx.fillStyle = '#111'; px(-3, -14, 2, 2); px(1, -14, 2, 2);
        ctx.fillStyle = '#333'; px(-3, -15, 2, 1); px(1, -15, 2, 1);
        ctx.fillStyle = '#fff'; px(-2, -10, 4, 1);
        ctx.fillStyle = '#8D6E4C'; px(-1, -8, 2, 2);

        // Torso
        ctx.fillStyle = '#FFB74D'; px(-5, -6, 10, 3);
        ctx.fillStyle = '#FF9800'; px(-5, -3, 10, 3);
        ctx.fillStyle = '#F57C00'; px(-5, 0, 10, 2);

        // Sleeves
        ctx.fillStyle = '#FFB74D'; px(-7, -6, 2, 3); px(5, -6, 2, 3);
        ...
        // Hands
        if (ch.drinking) {
            // Right hand holds drink
            ctx.fillStyle = '#FFD54F';
            ctx.fillRect(-7 * S, (2 + armSwing) * S, 2 * S, 2 * S);
            // Drink in left hand (raised)
        ...
        // Shorts
        ctx.fillStyle = '#ccc'; px(-4, 2, 8, 3);
        ctx.fillStyle = '#bbb'; px(-4, 4, 3, 1); px(1, 4, 3, 1);
        ...
        if (ch.drinking) {
            ...
            ctx.fillText(ch.drinkEmoji, sx + 30, sy - 40 + drinkBob);
            // Sipping particles
            if (elapsed > 0.5 && elapsed < 2.5) {
                ctx.font = '12px serif';
                for (let i = 0; i < 3; i++) {
                    const py = sy - 60 - i * 15 - elapsed * 10;
        ...
        ctx.fillText('Mayowa', sx + 1, sy - 120 + Math.sin(t * 1.5) * 3 + 1);
        ctx.fillStyle = '#fff';
        ctx.fillText('Mayowa', sx, sy - 120 + Math.sin(t * 1.5) * 3);
    },
    ...
    showSpeech() {
        ...
        bubble.style.top = (this.character.y - 150) + 'px';
